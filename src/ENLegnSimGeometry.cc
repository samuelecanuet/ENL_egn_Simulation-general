...
G4VPhysicalVolume* ENLegnSimGeometry::Construct( ){
  // Initialize detec classes
  detecProp = new ENLegnSimMaterials(path_bin+"Materials.cfg");
  Vacuum = detecProp->GetMaterial("VacuumWorld");
  VacuumWorld = detecProp->GetMaterial("VacuumWorld");
  Air = detecProp->GetMaterial("Air");
  Si = detecProp->GetMaterial("Silicon");
